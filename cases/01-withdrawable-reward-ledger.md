...
회원이 회원을 모집하고, 모든 주문이 구매자와 그 위 2단계에 각자의 등급률로 적립됩니다. 별도의 Partner 등급은 잔액을 연결된 지급 계정으로 **현금 인출**할 수 있습니다. 마지막 문장이 이걸 포인트 기능이 아니라 **원장 문제**로 만듭니다. 잔액이 현금으로 시스템을 떠나는 순간, 그걸 만든 모든 항목이 방어 가능해야 합니다.

**어려웠던 지점**

- **주문 하나가 여러 사람에게 서로 다른 요율로 여러 항목을 만듭니다.** 배송 하나로 구매자·업라인 2명·파트너까지 원장 4행이 생기고, 각각 독립적으로 틀릴 수 있습니다.
- **요율은 주문 단위가 아니라 수령자 단위입니다.** 각 수령자가 **자기 자신의** 현재 등급률로 적립받습니다. 그래서 한 번 계산해서 나눠줄 수 없고, 적립 시점에 수령자별로 해석해야 합니다.
- **모든 것이 되돌려질 수 있습니다.** 환불·취소·주문 수정이 적립을 되감아야 하며, 이미 일부 사용되거나 인출된 적립까지 포함합니다.
- **소수점 2자리, 그리고 통화 환산.** 정확히 float가 1센트 어긋난 잔액을 만드는 정밀도 구간이고, 여기서 1센트 차이는 문의 티켓이며 결국 감사 질문이 됩니다.

**접근**

- **Append-only, 절대 update 없음.** 원장은 행이 늘기만 합니다. 적립은 한 행, 취소는 원본을 참조하는 반대 부호의 다른 행, 사용은 또 한 행. 잔액은 그 합계입니다. 어떤 항목도 수정·삭제되지 않으므로 "이 잔액이 왜 이 숫자인가"의 답이 추론이 아니라 **항상 쿼리**입니다.
- **멱등성은 애플리케이션 로직이 아니라 DB 제약으로.** 항목 키를 주문·수령자·적립 종류로 만들고 unique 인덱스를 겁니다. 재전송된 웹훅은 DB가 거절하는 insert가 됩니다. 레이스가 끼어들 read-then-write 구간이 없고, "이미 처리했나"를 낡을 수 있는 쿼리로 답하는 경로도 없습니다. **이게 결정적인 선택입니다.** 애플리케이션 레벨 dedup은 웹훅 두 건이 동시에 처리되기 전까지만 맞고, 그 동시 처리는 정확히 재시도 폭주 때 일어납니다.
- **최소 단위 정수, 표시할 때만 변환.** 금액은 센트 `int64`, 포인트는 ×100 `int64` (3.33pt → 333). 적립·취소·인출 경로 어디에도 float가 없고, 십진 변환은 API 엣지 시리얼라이저에서만 합니다.
- **요율은 적립 시점에 읽는 설정값.** 트리 적립 등급률은 관리자 편집 가능, 정책 고정 요율은 규칙 엔진 상수. 둘 다 한 서비스에 있습니다. 요율 변경은 이후 적립부터 적용되고 기존 항목을 소급 수정하지 않습니다 — 그 항목들은 **당시 실제로 약속된 값**을 기록한 것이므로 그게 맞습니다.
- **원장 1개 + 인출은 권한.** 원래는 스토어 크레딧 / 현금화 포인트 2개 원장이었습니다. 이 분리는 "누가 현금화할 수 있는가"라는 **정책** 질문을 **데이터 모델**에 새긴 것이라, 정책이 바뀔 때마다 데이터 이관이 되고 잔액 조회마다 두 테이블 union이 됐습니다. 단일 원장 + 파트너 권한 판정으로 접은 게 이 시스템에서 가장 레버리지 큰 변경이었습니다. **현금화는 인가 문제이고, 인가 계층에 속합니다.**
- **적립은 결제가 아니라 배송 시점에.** 결제됐지만 출고되지 않은 주문이 세 사람에게 보상을 지급하면 안 됩니다. 트리거를 배송(로컬 픽업은 픽업 완료)으로 옮기면 보상이 물건을 따라가고, 되감기가 일상이 아니라 예외가 됩니다.

**결과**

- 중복 적립을 애플리케이션 체크가 아니라 DB 계층에서 차단 — 재전송 웹훅은 거절된 insert
- 원장 2개를 1개로 통합, 과거 잔액 이관, 인출은 권한 체크로 축소
- 회원별·주문별 전체 적립 이력 조회 가능 — 어떤 잔액도 항목 단위로 설명 가능
- 값 경로 전체에 부동소수 연산 없음

**다시 한다면** — 잔액을 원장 합산으로 구합니다. 맞고 계속 맞지만 회원 이력에 선형이고 스냅샷이 없습니다. 주기적 체크포인트 행(특정 시점 잔액 + 이후 항목만 합산)을 두면 감사 추적을 유지하면서 읽기 비용에 상한이 생깁니다. 원장이 커진 **뒤가 아니라 전에** 해야 할 일입니다.
